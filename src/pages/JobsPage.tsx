...
import { Briefcase, Grid3x3, List as ListIcon, Image, RefreshCw } from 'lucide-react';
...
export const JobsPage: React.FC = () => {
  ...
  const jobCategory = SPECIAL_CATEGORIES.find(cat => cat.type === 'job');
  ...
  const loadJobs = async () => {
    ...
    try {
      if (!jobCategory) {
        console.error('Job category not found');
        setLoading(false);
        return;
      }
      ...
      const { data: itemIds, error: rpcError } = await supabase
        .rpc('search_items_with_attributes', {
          p_category_id: jobCategory.id,
          p_filters: []
        });

      if (rpcError) {
        console.error('Error loading jobs:', rpcError);
        setLoading(false);
        return;
      }
      ...
      <Container maxWidth="xl" sx={{ flexGrow: 1, py: 3 }}>
        ...
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              {loading ? 'Lädt...' : `${items.length} ${items.length === 1 ? 'Job' : 'Jobs'}`}
            </Typography>
          </Box>
        </Box>
        ...
        {loading ? (
          ...
          <Box sx={{ textAlign: 'center', py: 8 }}>
            <Briefcase size={64} color={theme.palette.text.disabled} />
