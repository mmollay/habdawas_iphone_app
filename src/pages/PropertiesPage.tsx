...
import { Home, Grid3x3, List as ListIcon, Image, RefreshCw } from 'lucide-react';
...
export const PropertiesPage: React.FC = () => {
  ...
  const propertyCategory = SPECIAL_CATEGORIES.find(cat => cat.type === 'property');
  ...
  const loadProperties = async () => {
    ...
    try {
      if (!propertyCategory) {
        console.error('Property category not found');
        setLoading(false);
        return;
      }
      ...
      const { data: itemIds, error: rpcError } = await supabase
        .rpc('search_items_with_attributes', {
          p_category_id: propertyCategory.id,
          p_filters: []
        });

      if (rpcError) {
        console.error('Error loading properties:', rpcError);
        setLoading(false);
        return;
      }
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <Container maxWidth="xl" sx={{ flexGrow: 1, py: 3 }}>
        ...
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          <Home size={32} color={theme.palette.primary.main} />
          <Box sx={{ flexGrow: 1 }}>
            <Typography variant="h4" sx={{ fontWeight: 700 }}>
              Immobilien
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {loading ? 'Lädt...' : `${items.length} ${items.length === 1 ? 'Immobilie' : 'Immobilien'}`}
            </Typography>
          </Box>
        </Box>
        ...
        {loading ? (
          ...
          <Box sx={{ textAlign: 'center', py: 8 }}>
            <Home size={64} color={theme.palette.text.disabled} />
            <Typography variant="h6" sx={{ mt: 2, color: 'text.secondary' }}>
              Noch keine Immobilien verfügbar
            </Typography>
