...
import { CheckCircle, User, MapPin, Settings, Camera, Upload, Image as ImageIcon, Bell, Gift } from 'lucide-react';
...
const steps = ['Profil', 'Adresse', 'Benachrichtigungen', 'Geburtsdaten', 'Einstellungen'];
...
export function OnboardingWizard({ open, onComplete }: OnboardingWizardProps) {
  ...
  const [notificationData, setNotificationData] = useState({
    notify_push_messages: true,
    notify_email_transactional: true,
  ...
  const loadUserData = async () => {
    ...
    if (profile) {
      ...
      setNotificationData({
        notify_push_messages: profile.notify_push_messages ?? true,
        notify_email_transactional: profile.notify_email_transactional ?? true,
      ...
      if (activeStep === 0) {
        ...
        const { error: upsertError } = await supabase
          ...
          .upsert({
            id: user.id,
            email: user.email || '',
            notify_push_messages: notificationData.notify_push_messages,
            notify_email_transactional: notificationData.notify_email_transactional,
            newsletter_subscribed: notificationData.newsletter_subscribed,
          }, {
            onConflict: 'id'
          });
        ...
        // Save birth data (optional fields)
        ...
        if (birthData.birth_date) {
          updateData.birth_date = birthData.birth_date;
        }
        if (birthData.birth_time) {
          updateData.birth_time = birthData.birth_time;
        }
        if (birthData.birth_place) {
          updateData.birth_place = birthData.birth_place;
        }
        ...
        // Final step - save preferences and mark onboarding complete
        const { error: upsertError } = await supabase
          .from('profiles')
          .upsert({
            id: user.id,
        ...
        try {
          ...
          const userName = profileData?.full_name || user.email?.split('@')[0] || 'HabDaWas User';
          ...
          // Don't throw - welcome email is not critical for onboarding completion
        }
      }
      return true;
    } catch (err) {
  ...
  const getStepIcon = (step: number) => {
    ...
    if (step === 2) return <Bell size={24} />;
    if (step === 3) return <Gift size={24} />;
      ...
      <DialogContent sx={{ px: isMobile ? 2 : 4, py: isMobile ? 2 : 3 }}>
          ...
          <Box>
            ...
            >
              <FormControlLabel
                control={
                  <Switch
                    checked={notificationData.notify_push_messages}
                    onChange={(e) =>
                      setNotificationData({
                        ...notificationData,
                        notify_push_messages: e.target.checked,
                      })
                    }
                  />
                }
                label={
                  <Box>
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      Push-Benachrichtigungen für Nachrichten
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Erhalten Sie sofort Bescheid, wenn Sie eine neue Nachricht bekommen
                    </Typography>
                  </Box>
                }
              />
            ...
            >
              <FormControlLabel
                control={
                  <Switch
                    checked={notificationData.notify_email_transactional}
                    onChange={(e) =>
                      setNotificationData({
                        ...notificationData,
                        notify_email_transactional: e.target.checked,
                      })
                    }
                  />
                }
                label={
                  <Box>
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      Wichtige Account-Updates per E-Mail
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Bestellbestätigungen, Sicherheitshinweise und andere wichtige Informationen
                    </Typography>
                  </Box>
                }
              />
            ...
            >
              <FormControlLabel
                control={
                  <Switch
                    checked={notificationData.newsletter_subscribed}
                    onChange={(e) =>
                      setNotificationData({
                        ...notificationData,
                ...
                label={
                  <Box>
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      Newsletter mit Angeboten und Tipps
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Erhalten Sie Informationen über neue Features, Tipps und spezielle Angebote
                    </Typography>
                  </Box>
                }
              />
            ...
            <Alert severity="info" sx={{ mt: 2 }}>
              ...
              Wichtige transaktionale E-Mails sind für die Nutzung des Dienstes erforderlich.
            </Alert>
          </Box>
        )}
        ...
        {activeStep === 3 && (
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
              Diese Angaben sind freiwillig und ermöglichen besondere Features
            </Typography>
            <Typography variant="caption" color="text.secondary" sx={{ mb: 3, display: 'block', fontStyle: 'italic' }}>
              Sie erhalten an Ihrem Geburtstag eine kleine Überraschung von uns 🎁
            </Typography>
            ...
            <TextField
              ...
              helperText="Für Geburtstagswünsche und zukünftige Sternzeichen-Features"
              sx={{ mb: 2 }}
            />
            ...
            <TextField
              ...
              helperText="Für präzise astrologische Berechnungen (in Planung)"
              sx={{ mb: 2 }}
            />
            ...
            <TextField
              ...
              onChange={(e) =>
                setBirthData({ ...birthData, birth_place: e.target.value })
              }
              placeholder="z.B. Wien, Österreich"
              helperText="Für geografische astrologische Berechnungen (in Planung)"
              sx={{ mb: 2 }}
            />

            <Alert severity="success" sx={{ mt: 2 }}>
              Diese Angaben werden vertraulich behandelt und nicht öffentlich angezeigt.
              Sie können die Geburtstagsbenachrichtigungen jederzeit in den Einstellungen deaktivieren.
            </Alert>
          </Box>
        )}
