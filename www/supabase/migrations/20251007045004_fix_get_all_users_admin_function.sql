/*
  ...
  1. Changes
    - Fix message count query to use conversations instead of messages.receiver_id
    - Count messages properly through conversation relationships
*/
...
BEGIN
  ...
  GROUP BY p.id, au.email, p.full_name, p.phone, p.created_at, p.is_suspended, 
           p.suspended_at, p.suspended_reason, p.is_admin, ut.balance
