...
import { MapPin, Calendar, Heart, Send, Package, Truck, ZoomIn, Tag, Ruler, Weight, Box as BoxIcon, Palette, Sparkles, Grid3x3, Hash, Share2, X, ChevronLeft, ChevronRight, MessageCircle, ArrowUp, Link, Mail, Printer, Phone, Pencil, Trash2, Check, Image as ImageIcon, ShieldCheck } from 'lucide-react';
...
import { ItemModerationBar } from './ItemModerationBar';
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#fafafa' }}>
      ...
      <Container maxWidth="lg" sx={{ py: isMobile ? 2 : 3, px: isMobile ? 0 : 3, pt: !isMobile && allItems.length > 0 ? 14 : (isMobile ? 2 : 3) }}>
        ...
        <ItemModerationBar
          ...
          onStatusChange={() => {
            setItem({ ...item, status: item.status === 'active' ? 'inactive' : 'active' });
          }}
        />
          ...
          >
            {isMobile && (
              <>
                ...
                {allItems.length === 0 && <Box sx={{ height: '400px' }} />}
                {allItems.length > 0 && <Box sx={{ height: '400px' }} />}
                <Box
                  sx={{
                    position: 'sticky',
