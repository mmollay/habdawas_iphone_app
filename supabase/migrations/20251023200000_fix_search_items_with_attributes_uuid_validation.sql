/*
  # Fix search_items_with_attributes to handle non-UUID attributeIds gracefully

  The function was failing when receiving general filters (brand, color, condition, material)
  because these use string keys instead of UUIDs. This migration adds UUID validation
  to skip non-UUID attributeIds gracefully.
*/

-- Drop existing function
DROP FUNCTION IF EXISTS search_items_with_attributes(UUID, JSONB);

-- Recreate function with UUID validation
...
BEGIN
  ...
  LOOP
    ...
    filter_attribute_id_text := filter->>'attributeId';

    -- Skip if attributeId is not a valid UUID (e.g., general filters like "color", "brand")
    -- These are handled client-side
    BEGIN
      filter_attribute_id := filter_attribute_id_text::UUID;
    EXCEPTION WHEN invalid_text_representation THEN
      -- Not a valid UUID, skip this filter
      CONTINUE;
    END;
...
-- Add comment
COMMENT ON FUNCTION search_items_with_attributes IS 'Filters items by category and custom attributes. Returns matching item IDs. Gracefully handles non-UUID attributeIds.';
